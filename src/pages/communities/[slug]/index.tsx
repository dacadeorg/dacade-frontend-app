...
export const getServerSideProps = wrapper.getServerSideProps((store) => async (data) => {
  ...
  const getCurrentCommunty = store.dispatch(fetchCurrentCommunity(fetchArgs));
  const getAllCourses = store.dispatch(fetchAllCourses(fetchArgs));
  const getAllScoreboards = store.dispatch(fetchAllScoreboards(fetchArgs));

  const results = await Promise.all([getCurrentCommunty, getAllCourses, getAllScoreboards]);
