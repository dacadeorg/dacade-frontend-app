...
export const getServerSideProps = wrapper.getServerSideProps((store) => async (data) => {
  ...
  const getCurrentCommunty = store.dispatch(fetchCurrentCommunity({ ...fetchArgs, slug }));
  const getCurrentCourse = store.dispatch(fetchCourse({ ...fetchArgs, slug: course_slug }));

  const results = await Promise.all([getCurrentCommunty, getCurrentCourse]);
