...
export default function SubmissionCard({ submission }: SubmissionCard): ReactElement {
  ...
  return (
    <div className="bg-gray-50 text-sm lg:text-base text-gray-700 border-solid border border-gray-200 rounded-3xl mb-5 md:mb-0">
        ...
        <div className="p-7">
          {membersWithOrganiser?.length ? (
              ...
              {membersWithOrganiser?.map((user, index) => {
                return (
                  <div className="flex items-center space-x-1.5 pb-1.5 pt-1" key={`team-member-${index}`}>
                    <span className="text-sm lg:text-base leading-6 font-medium text-gray-900 pb-1 whitespace-nowrap">{user.displayName}</span>
                    {user.reputation ? (
            ...
            <div className="flex items-center space-x-1.5 pb-1.5 pt-1">
              <span className="text-sm lg:text-base leading-6 font-medium text-gray-900 pb-1">{submission.user.displayName}</span>
          ...
          <span className="block text-sm lg:text-base leading-snug text-gray-700 pb-4">
