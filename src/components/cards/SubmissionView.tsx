...
export default function SubmissionViewCard({ submission }: SubmissionViewCardProps): ReactElement {
  ...
  useEffect(() => {
    if (submission?.team) {
      const { organizer, members: teamMembers } = submission.team;
      if (teamMembers) setMembers([organizer || submission?.user, ...teamMembers.map(({ user }) => user)]);
    }
  }, [authUser, submission]);
  ...
  return (
    <UserCard
      ...
      expanded
    >
      <TranslationBox text={submission.text} defaultLocale={language} textCssClasses="text-base md:text-lg leading-normal text-gray-700" />
